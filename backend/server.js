...
// New route to send report email with PDF attachment
...
app.post("/api/send-report-email", async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: "safstreet3@gmail.com",
      subject: "Accepting the report. Please find the report attached.",
      text: `Please find the attached report for location: ${report.location}`,
      attachments: [
        {
          filename: `Damage_Report_${report._id || "report"}.pdf`,
          content: pdfBuffer,
...
app.get("/", (req, res) => {
  res.send("Welcome to the SafeStreet API!");
});
